...
export default function TilesDemo() {
  ...
  const gridCells = useMemo(() => {
    ...
    for (let gy = -mid; gy < GRID_SIZE - mid; gy++) {
      const stepLon =
        STEP_LAT / Math.max(0.2, Math.cos((center.lat * Math.PI) / 180));
  ...
  async function onBuy(cell) {
    if (!userId)
      return alert("Primero crea/carga un usuario en Demo usuario.");
  ...
  // Estilos inline (sin Tailwind) para el grid
  const styles = {
    legendDot: (color) => ({
    ...
    <div className="page-container">
      ...
      <div className="card">
        ...
        <div style={{ marginTop: 12, fontSize: 14, opacity: 0.85 }}>
          Parcelas visibles: {gridCells.length} · Ocupadas cerca:{" "}
          {occupied.length}
        </div>

        <div style={{ marginTop: 8, fontSize: 12, opacity: 0.6 }}>
          (El grid muestra posibles celdas. Las ocupadas provienen de la BD
          y se “pintan” encima. Haz click en una libre para comprar.)
        </div>
      </div>
